...
function SelectInput({ value, onChange, options, placeholder = 'Select...' }: { value: string; onChange: (value: string) => void; options: string[]; placeholder?: string }) {
  return (
...
function PhotoUpload({ label, hint, required, files, onChange }: { label: string; hint: string; required?: boolean; files: File[]; onChange: (files: File[]) => void }) {
      ...
      {files.length > 0 && (
        <ul className="mt-3 flex flex-wrap gap-2">
          {files.map((file, index) => (
            <li key={`${file.name}-${index}`} className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-canvas px-3 py-1 text-xs font-semibold text-slate-600">
              {file.name}
...
function ConsentCheckbox({ checked, onChange, children }: { checked: boolean; onChange: (value: boolean) => void; children: ReactNode }) {
  return (
...
function RegisterComplaintPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <Link to="/public/dashboard" className="inline-flex items-center gap-1.5 text-sm font-bold text-brand-700 hover:text-brand-800 dark:text-brand-300">
